...
CRON_ENTRY_REPORT="*/30 9-18 * * 1-5 cd $PROJECT_DIR && $UV_EXE run python -m src.report >> $LOG_FILE 2>&1"
CRON_ENTRY_MAILING_LIST="45 9,13,17 * * 1-5 cd $PROJECT_DIR && $UV_EXE run python -m src.report_mailing_list >> $LOG_FILE 2>&1"
CRON_ENTRY_RECAP="0 8 * * * cd $PROJECT_DIR && $UV_EXE run python -m src.recap >> $LOG_FILE 2>&1"
...
echo ""
echo "# Janus - digest report every 30 min 9-18 (Mon-Fri)"
echo "$CRON_ENTRY_REPORT"
echo ""
echo "# Janus - mailing list report at 9:45, 13:45, 17:45 (Mon-Fri)"
echo "$CRON_ENTRY_MAILING_LIST"
echo ""
echo "# Janus - out-of-office recap daily at 8:00"
echo "$CRON_ENTRY_RECAP"
echo ""
